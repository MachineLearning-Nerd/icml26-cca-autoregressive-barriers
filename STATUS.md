...
- C3 preliminary producer: all 254 `(ell,z)` cells for `ell=1..7` solve the independently formulated exact augmentation LP with credit probability `gamma=0.4` (maximum error `3.22e-15`), and exhaustive marked-string decision trees require exactly `2^ell` worst-case oracle queries. The asymptotic reduction still needs its independent proof audit.
