"""Write a deterministic hash manifest for public audit artifacts."""
...
OUT = ROOT / "outputs/artifact_manifest.json"
EXCLUDED = {
...
def main() -> None:
    ...
    for path in sorted(ROOT.rglob("*")):
        ...
        if (
            ...
            or any(part in {".git", ".venv", ".pytest_cache", ".trackio", "__pycache__"} for part in path.parts)
            or path.suffix == ".pyc"
        ):
            continue
    ...
    print(json.dumps({"path": "outputs/artifact_manifest.json", "files": len(records), "sha256": sha256(OUT)}, sort_keys=True))
